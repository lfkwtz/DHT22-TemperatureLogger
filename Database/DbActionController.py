...
class DbController():
	...
	def getLastSensorMailSentTime(self,sensor):
		...
		sqlQuery = "SELECT * FROM emaillog WHERE triggedsensor='%s' AND sendtime IN (SELECT max(sendtime)FROM emaillog WHERE triggedsensor='%s')" % (sensor,sensor)
	...
	def setLastSensorMailSentTime(self,sensor,sendingTime,humidity,temperature):
		...
		sqlQuery = "INSERT INTO emaillog SET sendtime='%s', triggedsensor='%s', triggedlimit='%s' ,lasttemperature='%s'" % (sendingTime,sensor,humidity,temperature)
	...
	def getLastMailSentTime(self,action):
		...
		sqlQuery = "SELECT MAX(sendtime) FROM emaillog WHERE triggedsensor ='%s'" % (action)
	...
	def setLastMailSentTime(self,timestamp,action):
		...
		# Sql query to be executed
		sqlQuery = "INSERT INTO emaillog SET sendtime='%s', triggedsensor='%s', triggedlimit='%s' ,lasttemperature='%s'" % (timestamp,action,"","")
	...
	def getLastSensorMeasurements(self,sensor):
		...
		# Sql query to be executed
		sqlQuery = "SELECT * FROM tempdata WHERE sensor='%s' and dateandtime IN (SELECT max(dateandtime) FROM tempdata WHERE sensor='%s')" % (sensor,sensor)
	...
	def getWeeklyAverageTemp(self,sensor):
		...
		try:
			...
			sqlQuery = "SELECT AVG(temperature) FROM tempdata WHERE dateandtime BETWEEN '%s' AND '%s' AND sensor='%s'" % (delta,self.currentTimeAsString,sensor)
	...
	def getWeeklyAverageHumidity(self,sensor):
		...
		try:
			# Query to be executed
			sqlQuery = "SELECT AVG(humidity) FROM tempdata WHERE dateandtime BETWEEN '%s' AND '%s' AND sensor='%s'" % (delta,self.currentTimeAsString,sensor)
	...
	def setSensorTemperatureAndHumidityToDb(self,sensor,sensorData):
		...
		try:
			# Query to be executed
			sqlQuery = "INSERT INTO tempdata SET dateandtime='%s', sensor='%s', temperature='%s', humidity='%s'" % (self.currentTimeAsString,sensor,temperature,humidity)
